...
bash /home/cjuarez/42_minishell_tester/tester.sh $test_mode | sed -e 's/\x1b\[[0-9;]*m//g' | while IFS= read -r line; do
    if [[ $line =~ 🚀|# ]]; then
        echo "$line"  # Print headers and subheaders
    elif [[ $line =~ 🏁 ]]; then
        inSummary=true  # Flag to indicate summary sectiocdn start
        echo "$line"
